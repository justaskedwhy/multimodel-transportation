...
# def process_status(jod_id,status):

def process_data():
    jsondata = request.get_json()
    order_data = jsondata['order_data']
    data = jsondata['route_data']

    processed_data = process_data_func(order_data, data)

    return "Data received successfully and processing"

def process_data_func(order_data, data):
    nodes = pd.unique(data[['Source', 'Destination']].values.ravel('k')).tolist()
    travelmodes = data['Travel Mode'].unique().tolist()
    ...
    def calvalue(route_info, volume_ut, weight_ut, total_ut, ratio, order_value):
        ...
        # method 2
        dict['VariableFreightCost'] = [route_info['VariableFreightCost'] * np.max((np.ceil(
            (volume_ut * route_info['Container Size']) * route_info['VolumetricWeightConversionFactor']), np.ceil(
            weight_ut * route_info['MaxWeightPerEquipment'])))]
        # method 3
    ...
    def variablefinder(travelmode, carrier, initial, final):
        variable = {}
        if not ((data['Travel Mode'] == travelmode) & (data['Carrier'] == carrier) & (data['Source'] == initial) & (
                data['Destination'] == final)).any():  # checks whether a row exist in the dataframe
            variable['transit_time'] = 0
            return variable
        # dataslice is a specific row which contain data specific to (travelmode,initial,final)
        dataslice = data.loc[
            (data['Travel Mode'] == travelmode) & (data['Source'] == initial) & (data['Destination'] == final) & (
    ...
    def route(n, nid, date, ini, fin, volume, weight, order_value,
              finaldat=()):  # finaldat is in tuple because of the problems with list(local and globle variable problems)
        if n == 0:
            for travelelement in travelmodes:
                for carrierelement in carriermodes:
                    ...
                    if variable['transit_time']:
                        ...
                        total_time = variable['custom_clearance_time'] + variable['Port_Airport_RailHandling_Time'] + \
                                     variable['extra_time'] + variable['transit_time']
                ...
                for travelelement in travelmodes:
                    for carrierelement in carriermodes:
                        ...
                        if variable['transit_time']:
                            ...
                            Weight_Utilitation = weight / MaxWeightPerEquipment
                            Volume_Utilization = volume / container_size
                            total_time = variable['custom_clearance_time'] + variable[
                                'Port_Airport_RailHandling_Time'] + variable['extra_time'] + variable['transit_time']
                            date_new = date - datetime.timedelta(hours=total_time)
                            week_new = date_new.strftime('%Y-%V')
                            pt.append((source, destination, travelelement, carrierelement, container_size,
                                       MaxWeightPerEquipment, VolumetricWeightConversionFactor, Weight_Utilitation,
                                       Volume_Utilization, order_value, total_time, date_new, week_new))
                            for travelelement2 in travelmodes:
                                for carrierelement2 in carriermodes:
                                    ...
                                    if variable2['transit_time']:
                                        ...
                                        Weight_Utilitation = weight / MaxWeightPerEquipment
                                        Volume_Utilization = volume / container_size
                                        total_time = variable2['custom_clearance_time'] + variable2[
                                            'Port_Airport_RailHandling_Time'] + variable2['extra_time'] + variable2[
                                                         'transit_time']
                                        date_new_2 = date_new - datetime.timedelta(hours=total_time)
                                        week_new_2 = date_new_2.strftime('%Y-%V')
                                        pt.append((source, destination, travelelement2, carrierelement2, container_size,
                                                   MaxWeightPerEquipment, VolumetricWeightConversionFactor,
                                                   Weight_Utilitation, Volume_Utilization, order_value, total_time,
                                                   date_new_2, week_new_2))
                                        t.append(
                                            tuple(pt[::-1]))  # to change the order from last to first to first to last.
                    ...
                    for carrierelement in carriermodes:
                        ...
                        if variable['transit_time']:
                            ...
                            pt.append((source, destination, travelelement, carrierelement, container_size,
                                       MaxWeightPerEquipment, VolumetricWeightConversionFactor, Weight_Utilitation,
                                       Volume_Utilization, order_value, total_time, date_new, week_new))
                            route(n - 1, pc_new(nid.copy(), (intermediate,)), date_new, ini, intermediate, volume,
                                  weight, order_value, tuple(pt))
    ...
    def consolidation_0(zero_routes):  # for only the routes having zero intermidiates, done in DemandPullAhead method
        if len(zero_routes) == 0:
            return  # avoids error
        one_stop_df = pd.DataFrame(zero_routes, columns=['Order', 'Source', 'Destination', 'Travel_Mode', 'Carrier',
        ...
        for slice in range(1, one_sort.shape[0]):
            if one_sort.loc[slice, 'Date'] >= start + datetime.timedelta(
                    days=pullahead):  # checks if the next routes is more that pullahead days from the present route
                break
        ...
        if start_dict['Weight_Utilitation'] >= start_dict['Volume_Utilization']:
            ...
            last_dict['Volume_Utilization'] = last_volumn_Ut
        start_dict['DemandPullAhead'] = True
        t_consolidate_0.append(
            (tuple(start_dict.values())[0], (tuple(start_dict.values())[1:],)))  # ('orderindex',((route)))
        consolidation_0((tuple(last_dict.values()),) + zero_routes[slice1 + 1:])

    def consoildation(orderno, route, routedictionary, consolidant):
        fol = False
        pt = consolidant.copy()
        to_consolidate_df = pd.DataFrame(route,
                                         columns=['Source', 'Destination', 'Travel_Mode', 'Carrier', 'Container_Size',
                                                  'MWpE', 'VWcF', 'Weight_Utilitation', 'Volume_Utilization',
                                                  'order_value', 'Total_Time', 'Date',
                                                  'Week'])  # dataframes are easy to work with
        for orderindex in routedictionary:
            ...
            for root in routetuple:
                if fol:  # checks whether the order index has already used more than one time
                    fol = False
                    break
                from_df = pd.DataFrame(root,
                                       columns=['Source', 'Destination', 'Travel_Mode', 'Carrier', 'Container_Size',
                                                'MWpE', 'VWcF', 'Weight_Utilitation', 'Volume_Utilization',
                                                'order_value', 'Total_Time', 'Date', 'Week'])
                equaldf = to_consolidate_df.merge(from_df,
                                                  on=['Source', 'Destination', 'Travel_Mode', 'Carrier', 'Week'],
                                                  suffixes=('', '_DROP')).filter(
                    regex='^(?!.*_DROP)')  # finds commen rows to both dataframe
                for i in range(len(equaldf)):
                    fol = True
                    equalrow = equaldf.loc[i]
                    index = pt.index[(consolidant['Source'] == equalrow['Source']) & (
                                consolidant['Destination'] == equalrow['Destination']) & (
                                                 consolidant['Travel_Mode'] == equalrow['Travel_Mode']) & (
                                                 consolidant['Carrier'] == equalrow['Carrier']) & (
                                                 consolidant['Week'] == equalrow['Week'])]
                    pt.loc[index, 'Consolidant'] += str(
                        (orderindex, routetuple.index(root))) + ','  # adds the route index to the column consolidation
        ...
        for orderindex in routes:
            for route in routes[orderindex]:
                if len(route) == 1:
                    ...
                    one_stop[(orderindex[0], route[0][2], route[0][3])] += [
                        ('{}'.format((orderindex)),) + (route[0])]  # ('orderindex',....,...,..)
                df = pd.DataFrame(route,
                                  columns=['Source', 'Destination', 'Travel_Mode', 'Carrier', 'Container_Size', 'MWpE',
                                           'VWcF', 'Weight_Utilitation', 'Volume_Utilization', 'order_value',
                                           'Total_Time', 'Date', 'Week'])
                df['Consolidant'] = ''
        ...
        for orderindex in route_dict_con:
            ...
            for routes in route_dict_con[orderindex]:
                ...
                for routeslice in routes:
                    if routeslice[-1]:
                        routeslice_pd = pd.DataFrame((routeslice,),
                                                     columns=['Source', 'Destination', 'Travel_Mode', 'Carrier',
                                                              'Container_Size', 'MWpE', 'VWcF', 'Weight_Utilitation',
                                                              'Volume_Utilization', 'order_value', 'Total_Time', 'Date',
                                                              'Week', 'DemandPullAhead'])
                    else:
                        routeslice_pd = pd.DataFrame((routeslice,),
                                                     columns=['Source', 'Destination', 'Travel_Mode', 'Carrier',
                                                              'Container_Size', 'MWpE', 'VWcF', 'Weight_Utilitation',
                                                              'Volume_Utilization', 'order_value', 'Total_Time', 'Date',
                                                              'Week', 'Consolids', 'DemandPullAhead'])
                    try:
                    ...
                    for consolid_i in range(len(consolids)):
                        ...
                        consolid_pd = pd.DataFrame(route_dict[consolidable[0]][consolidable[1]],
                                                   columns=['Source', 'Destination', 'Travel_Mode', 'Carrier',
                                                            'Container_Size', 'MWpE', 'VWcF', 'Weight_Utilitation',
                                                            'Volume_Utilization', 'order_value', 'Total_Time', 'Date',
                                                            'Week'])
                        index = consolid_pd.index[(consolid_pd['Source'] == routeslice_pd['Source'].item()) & (
                                    consolid_pd['Destination'] == routeslice_pd['Destination'].item()) & (
                                                              consolid_pd['Travel_Mode'] == routeslice_pd[
                                                          'Travel_Mode'].item()) & (
                                                              consolid_pd['Carrier'] == routeslice_pd[
                                                          'Carrier'].item()) & (
                                                              consolid_pd['Week'] == routeslice_pd['Week'].item())]
                    ...
                    if total_ut == total_weight_ut:
                        ...
                        route_index = data.index[((data['Source'] == routeslice_pd['Source'].item()) & (
                                    data['Destination'] == routeslice_pd['Destination'].item()) & (
                                                              data['Travel Mode'] == routeslice_pd[
                                                          'Travel_Mode'].item()) & (
                                                              data['Carrier'] == routeslice_pd['Carrier'].item()))]
                        route_ = data.loc[route_index].to_dict('records')
                        cost_pd = calvalue(route_[0], routeslice_pd['Volume_Utilization'].item(),
                                           routeslice_pd['Weight_Utilitation'].item(), total_ut, ratio, orderindex[1])
                    else:
    ...
    def display(dictionary, routedict={}):
        ...
        # Insert data into RouteTool_Routing table
        for orderindex_ in routedict:
            for route_ in routedict[orderindex_]:
                ...
                # Insert row into RouteTool_Routing table
                columns = ', '.join(finaldat_.keys())
                values = tuple(finaldat_.values())
                query = f"INSERT INTO RouteTool_Routing ({columns}) VALUES {values}"
                cursor.execute(query)

        # Insert data into RouteTool_Cost table
        for orderindex in dictionary:
            for routes in dictionary[orderindex]:
                ...
                finaldat['Total Cost'] = finaldat['Fixed Freight Cost'] + finaldat['Port/Airport/Rail Handling Cost'] + \
                                         finaldat['Documentation Cost'] + finaldat['Equipment Cost'] + finaldat[
                                             'Extra Cost'] + \
                                         finaldat['VariableFreightCost'] + finaldat['Bunker/ Fuel Cost'] + finaldat[
                                             'Warehouse Cost'] + \
                                         finaldat['Transit Duty']

                # Insert row into RouteTool_Cost table
                columns = ', '.join(finaldat.keys())
                values = tuple(finaldat.values())
                query = f"INSERT INTO RouteTool_Cost ({columns}) VALUES {values}"
                cursor.execute(query)
    ...
    for inputslice in order_data.values.tolist():
        for n in range(4):
            route(n, pc_new(nodeindex, (inputslice[1], inputslice[2])), inputslice[7], inputslice[1], inputslice[2],
                  inputslice[5], inputslice[4], inputslice[3], ())
            try:
                week = inputslice[7].strftime('%Y-%V')
            except:
                week = None
        d_route[(inputslice[0], inputslice[3], inputslice[4], inputslice[5], week, inputslice[8])] = tuple(
            t)  # storing routes belonging to different orders with dictionary
        t.clear()
    # for i in d_route:
    #     print(i)
    #     for j in d_route[i]:
